...
int check_executor_permissions(char *executable_file)
{
  ...
  if ((filestat.st_mode & S_IROTH) == S_IROTH || (filestat.st_mode & S_IWOTH) == S_IWOTH || (filestat.st_mode & S_IXOTH) == S_IXOTH)
  {
    fprintf(LOGFILE,
...
int profile_oci_container(const char * user, int container_pid, const char* command_file) {
  if (container_pid <= 0) {
    fprintf(ERRORFILE, "The container pid is %d\n", container_pid);
    fflush(ERRORFILE);
    return UNABLE_TO_EXECUTE_CONTAINER_SCRIPT;
  }
  ...
  snprintf(nsenter_command_with_binary, command_size, "%s --target %d --mount --pid", nsenter_binary, container_pid);
