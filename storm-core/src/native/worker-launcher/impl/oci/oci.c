...
int run_oci_container(const char* command_file) {
  int rc = 0;
  ...
  rc = setup_container_paths(olc->username, olc->app_id, olc->container_id, work_dir, olc->script_path);
  if (rc != 0) {
    goto cleanup;
  }
  ...
  runc_config_path = write_oci_runc_config(olc, ctx->upper.mount_path);
  ...
  exec_runc(olc->container_id, runc_config_path, olc->pid_file);
