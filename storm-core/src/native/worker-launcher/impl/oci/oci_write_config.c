...
static cJSON* build_runc_config_process_user(const char* username) {
  ...
  struct storm_user_info* hui = storm_user_info_alloc();
  if (hui == NULL) {
    return NULL;
  }

  int rc = storm_user_info_fetch(hui, username);
  if (rc != 0) {
  ...
  rc = storm_user_info_getgroups(hui);
  if (rc != 0) {
 ...
 */
char* write_oci_runc_config(const oci_launch_cmd* olc,
    const char* rootfs_path) {
  ...
  char* runc_config_path = get_runc_config_path(olc->pid_file);
