...
static bool parse_oci_launch_cmd_layer(olc_layer_spec* layer_out,
  ...
  if (!cJSON_IsObject(layer_json)) {
    fputs("OCI launch command layer is not an object\n", ERRORFILE);
    return false;
  }
  ...
  if (!cJSON_IsString(media_type_json)) {
    fputs("Bad/Missing media type for OCI launch command layer\n", ERRORFILE);
    return false;
  }

  const cJSON* path_json = cJSON_GetObjectItemCaseSensitive(layer_json, "path");
  if (!cJSON_IsString(path_json)) {
    fputs("Bad/Missing path for OCI launch command layer\n", ERRORFILE);
    return false;
  }
...
static olc_layer_spec* parse_oci_launch_cmd_layers(unsigned int* num_layers_out,
  ...
  if (!cJSON_IsArray(layers_json)) {
    fputs("Bad/Missing OCI launch command layers\n", ERRORFILE);
    return NULL;
  }
...
static int parse_reap_layers_keep(cJSON* json) {
  if (!cJSON_IsNumber(json)) {
    fputs("Bad/Missing OCI reap layer keep number\n", ERRORFILE);
    return -1;
  }
...
static void parse_oci_launch_cmd_oci_config(oci_config* oc, cJSON* oc_json) {
  if (!cJSON_IsObject(oc_json)) {
    fputs("Bad/Missing OCI runtime config in launch command\n", ERRORFILE);
    return;
  }
  ...
  if (!cJSON_IsObject(process_json)) {
    fputs("Bad/Missing process section in OCI config\n", ERRORFILE);
    return;
  }
...
static bool is_valid_oci_config_linux_resources(const cJSON* oclr) {
  if (!cJSON_IsObject(oclr)) {
    fputs("OCI config linux resources missing or not an object\n", ERRORFILE);
    return false;
  }
  ...
  cJSON_ArrayForEach(e, oclr) {
    if (strcmp("blockIO", e->string) == 0) {
      ...
      fprintf(ERRORFILE,
          "Unrecognized OCI config linux resources element: %s\n", e->string);
...
static bool is_valid_oci_config_linux(const cJSON* ocl) {
  if (!cJSON_IsObject(ocl)) {
    fputs("OCI config linux section missing or not an object\n", ERRORFILE);
    return false;
  }
  ...
  cJSON_ArrayForEach(e, ocl) {
    if (strcmp("cgroupsPath", e->string) == 0) {
      ...
      fprintf(ERRORFILE, "Unrecognized OCI config linux element: %s\n",
          e->string);
      all_sections_ok = false;
    }
  }
...
static bool is_valid_mount_options(const cJSON* mo) {
  if (!cJSON_IsArray(mo)) {
    fputs("OCI config mount options not an array\n", ERRORFILE);
    return false;
  }
  ...
  cJSON_ArrayForEach(e, mo) {
    if (!cJSON_IsString(e)) {
      fputs("OCI config mount option is not a string\n", ERRORFILE);
      return false;
    }
  ...
  if (!has_rbind) {
    fputs("OCI config mount options missing rbind\n", ERRORFILE);
    return false;
  }
  if (!has_rprivate) {
    fputs("OCI config mount options missing rprivate\n", ERRORFILE);
    return false;
  }
...
static bool is_valid_mount(const cJSON* mount) {
  if (!cJSON_IsObject(mount)) {
    fputs("OCI config mount entry is not an object\n", ERRORFILE);
    return false;
  }
  ...
  cJSON_ArrayForEach(e, mount) {
    if (strcmp("type", e->string) == 0) {
      if (!cJSON_IsString(e) || strcmp("bind", e->valuestring)) {
        fputs("OCI config mount is not bind type\n", ERRORFILE);
        return false;
      }
      has_type = true;
    } else if (strcmp("source", e->string) == 0) {
      if (!cJSON_IsString(e)) {
        fputs("OCI config mount source is not a string\n", ERRORFILE);
        return false;
      }
      source = e->valuestring;
    } else if (strcmp("destination", e->string) == 0) {
      if (!cJSON_IsString(e)) {
        fputs("OCI config mount destination is not a string\n", ERRORFILE);
        return false;
      }
      ...
      fprintf(ERRORFILE, "Unrecognized OCI config mount parameter: %s\n",
          e->string);
      return false;
    }
  }
  ...
  if (!has_type) {
    fputs("OCI config mount missing mount type\n", ERRORFILE);
    return false;
  }
  if (!has_options) {
    fputs("OCI config mount missing mount options\n", ERRORFILE);
    return false;
  }
  if (source == NULL) {
    fputs("OCI config mount missing source\n", ERRORFILE);
    return false;
  }
  if (destination == NULL) {
    fputs("OCI config mount missing destination\n", ERRORFILE);
    return false;
  }
...
static bool is_valid_oci_config_mounts(const cJSON* ocm) {
  ...
  if (!cJSON_IsArray(ocm)) {
    fputs("OCI config mounts is not an array\n", ERRORFILE);
    return false;
  }
...
static bool is_valid_oci_config_process(const oci_config_process* ocp) {
  ...
  if (!cJSON_IsArray(ocp->args)) {
    fputs("OCI config process args is missing or not an array\n", ERRORFILE);
    return false;
  }
  ...
  cJSON_ArrayForEach(e, ocp->args) {
    if (!cJSON_IsString(e)) {
      fputs("OCI config process args has a non-string in array\n", ERRORFILE);
      return false;
    }
  }

  if (!cJSON_IsString(ocp->cwd)) {
    fputs("Bad/Missing OCI config process cwd\n", ERRORFILE);
    return false;
  }

  if (!cJSON_IsArray(ocp->env)) {
    fputs("OCI config process env is missing or not an array\n", ERRORFILE);
    return false;
  }
  cJSON_ArrayForEach(e, ocp->env) {
    if (!cJSON_IsString(e)) {
      fputs("OCI config process env has a non-string in array\n", ERRORFILE);
      return false;
    }
  }
...
static bool is_valid_oci_config(const oci_config* oc) {
  ...
  if (oc->hostname != NULL && !cJSON_IsString(oc->hostname)) {
    fputs("OCI config hostname is not a string\n", ERRORFILE);
    is_valid = false;
  }
...
static bool is_valid_oci_launch_cmd(const oci_launch_cmd* olc) {
  ...
  if (olc->username == NULL) {
    fputs("OCI command has bad/missing username\n", ERRORFILE);
    return false;
  }

  if (olc->app_id == NULL) {
    fputs("OCI command has bad/missing application ID\n", ERRORFILE);
    return false;
  }

  if (olc->container_id == NULL) {
    fputs("OCI command has bad/missing container ID\n", ERRORFILE);
    return false;
  }
  ...
  if (olc->pid_file == NULL) {
    fputs("OCI command has bad/missing pid file\n", ERRORFILE);
    return false;
  }
  struct stat statbuf;
  ...
  if (olc->script_path == NULL) {
    fputs("OCI command has bad/missing container script path\n", ERRORFILE);
    return false;
  }
...
oci_launch_cmd* parse_oci_launch_cmd(const char* command_filename) {
  ...
  char* app_id = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(
      olc_json, "applicationId"));
  if (app_id != NULL) {
    olc->app_id = strdup(app_id);
  }
