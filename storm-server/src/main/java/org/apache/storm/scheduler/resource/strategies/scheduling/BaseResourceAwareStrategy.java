...
public abstract class BaseResourceAwareStrategy implements IStrategy {
    ...
    protected List<ExecutorDetails> orderExecutors(
        ...
        Boolean orderByProximity = (Boolean) td.getConf().get(EXPERIMENTAL_TOPOLOGY_RAS_ORDER_EXECUTORS_BY_PROXIMITY_NEEDS);
        if (orderByProximity == null || !orderByProximity) {
            return orderExecutorsDefault(td, unassignedExecutors);
        } else {
            LOG.info("{} is set to true", EXPERIMENTAL_TOPOLOGY_RAS_ORDER_EXECUTORS_BY_PROXIMITY_NEEDS);
